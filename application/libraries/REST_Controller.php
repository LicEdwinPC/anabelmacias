...
abstract class REST_Controller extends CI_Controller
{
	...
	public function response($data = null, $http_code = null)
	{
		...
		$this->ci->load->model('CaUsuarios_model');
		$user = $this->CaUsuarios_model->get(array(
			'ca_usuarios.id' => $user_id
		));
			...
			// $insert = array(
			// 	'codigoEstatus' => $http_code,
			// 	'id' => utils::guid(),
			// 	'usuario_id' => $user_id,
			// 	'modulo' => $module,
			// 	'controlador' => $controller,
			// 	'funcion' => $method,
			// 	'parametroEntrada' => @json_encode(array('get' => $_GET, 'post' => $_POST)),
			// 	'parametroSalida' => (is_string($output) ? $output : json_encode($output)),
			// 	'id_Perfil' => $user['id_Perfil']
			// 	// 'fechaRegistro' => $fecha->format("Y-m-d H:i:s.u")
			// );
			// // utils::pre($insert);
			// $this->ci->db->insert('ci_LogsApp', $insert);
			// // utils::pre($this->ci->db->last_query());
	...
	protected function _parse_post()
	{
		...
		$this->request->contenido = @file_get_contents('php://input');
